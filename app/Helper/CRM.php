...
use App\Models\User;
use App\Models\UserCrmConnection;
...
class CRM
{
    ...
    public static function supersetting($key, $default = '')
    {
        ...
        $setting = Setting::where(['user_id' => $user->id ?? 1, 'key' => $key])->first();
        $value = $setting->value ?? $default;
        return $value;
    }
    public static function getDefault($key, $def = '', Agency $agency)
    {
        $setting = Setting::where(['agency_id' => $agency->id, 'key' => $key])->first();
    ...
    public static function getLocationAccessToken(Agency $agency, $location_id, $token = null, $retries = 0)
    {
        if (!$token) {
            $token = self::getCrmToken(['agency_id' => $agency->id, 'user_type' => self::$lang_com]);
        }
        $resp = null;
        if ($token) {
            ...
            if ($resp && property_exists($resp, 'access_token')) {
                $resp = self::saveCrmToken($resp, $agency->id);
            } else if (self::isExpired($resp) && $retries == 0) {
                list($is_refresh, $token) = self::getRefreshToken($agency->id, $token, true);
                if ($is_refresh) {
                    return self::getLocationAccessToken($agency->id, $location_id, $token, $retries + 1);
                }
            }
        }
        return $resp;
    ...
    public static function crm_token($code = '', $method = '', Agency $agency = null)
    {
        ...
        $data = [];
        $data['client_id'] = static::getDefault('crm_client_id', '', $agency);
        $data['client_secret'] = static::getDefault('crm_client_secret', '', $agency);
        $data[$md] = $code;
